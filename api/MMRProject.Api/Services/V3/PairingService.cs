    ...
    : IPairingService
{
    ...
    public async Task<PairingSubmitResponse> SubmitPairingAsync(PairingSubmitRequest request)
    {
        ...
        // Log the tag's own id, never the device-supplied RfidUid (log forging).
        if (existingTag != null && existingTag.UserId != pairingCode.UserId)
        {
            logger.LogInformation("Pairing submit rejected: tag {TagId} already paired to another user", existingTag.Id);
            return new PairingSubmitResponse { Success = false };
        }

        var tag = existingTag
                  ?? dbContext.RfidTags.Add(new RfidTag { UserId = pairingCode.UserId, RfidUid = request.RfidUid }).Entity;

        pairingCode.UsedAt = now;
        await dbContext.SaveChangesAsync();

        logger.LogInformation("Paired tag {TagId} to user {UserId}", tag.Id, pairingCode.UserId);
