...
public interface IMatchMakingService
{
  ...
  Task<string> GenerateTeamsAsync(List<string> chipIds);
}
...
) : IMatchMakingService
{
  ...
  public async Task<string> GenerateTeamsAsync(List<string> chipIds)
  {
    ...
    // Create set of team 1 player IDs for easy lookup
    var team1PlayerIds = result.Team1.Players.Select(p => p.Id).ToHashSet();

    // Create lookup for chipId -> user mapping
    var chipIdToUser = users.ToDictionary(u => u.ChipId, u => u);
    
    // Map the team assignments back to original chipId order
    var teams = chipIds
        .Select(chipId => team1PlayerIds.Contains(chipIdToUser[chipId].Id) ? "1" : "0");

    // Return teams as a string of comma separated 1 and 0
